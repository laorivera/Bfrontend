...
//store values
  ...
  set classSelection(value: number) {
    ...
    this.fetchList(`http://127.0.0.1:8080/pantslist/${this._classSelection}`);
  }
  ...
  onChangeRarity(event: number) {
    ...
    if (this.selectedItem?.name) {
      this.fetchRating(`http://127.0.0.1:8080/pantsratinglist/?itempants=${this.selectedItem.name}&rarityselect_pants=${this.selectedRarity}`);
    }
